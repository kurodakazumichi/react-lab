このプロジェクトは[Create React App](https://github.com/facebook/create-react-app)によって作られました。

以下に、基本的なタスクの実行方法について記述します。  
またここに書かれているガイドの最新版は[こちら](https://github.com/facebook/create-react-app/blob/master/packages/react-scripts/template/README.md)。
...
- [サポートされている言語機能](#supported-language-features)
- [エディタでのシンタックスハイライト](#syntax-highlighting-in-the-editor)
- [エディタでのLint出力の表示](#displaying-lint-output-in-the-editor)
- [エディタでのデバッグ](#debugging-in-the-editor)
- [自動コードフォーマット](#formatting-code-automatically)
...
- [画像、フォント、ファイルの追加](#adding-images-fonts-and-files)
- [SVGの追加](#adding-svgs)
- [publicフォルダーの使い方](#using-the-public-folder)
  - [HTMLの変更](#changing-the-html)
  - [ModulesSystem外へのアセットの追加](#adding-assets-outside-of-the-module-system)
  - [publicフォルダを使用する場合](#when-to-use-the-public-folder)
- [グローバル変数の使い方](#using-global-variables)
- [ブートストラップの追加](#adding-bootstrap)
  - [カスタムテーマの使い方](#using-a-custom-theme)
- [フローの追加](#adding-flow)
- [リレーの追加](#adding-relay)
...
- [カスタム環境変数の追加](#adding-custom-environment-variables)
  ...
  - [シェル内での一時環境変数の追加](#adding-temporary-environment-variables-in-your-shell)
  - [`.env`による開発環境変数の追加](#adding-development-environment-variables-in-env)
- [デコレータを使えますか？](#can-i-use-decorators)
- [Ajaxによるデータの取得](#fetching-data-with-ajax-requests)
- [APIバックエンドとの統合](#integrating-with-an-api-backend)
  - [Node](#node)
  - [Ruby on Rails](#ruby-on-rails)
- [開発環境でのAPIリクエストのプロキシ](#proxying-api-requests-in-development)
  - [Proxy設定後の"Invalid Host Header"エラー](#invalid-host-header-errors-after-configuring-proxy)
  - [プロキシの手動設定](#configuring-the-proxy-manually)
- [開発環境でHTTPSの使い方](#using-https-in-development)
...
- [デバッグテスト実行](#debugging-tests)
  ...
  - [Visual Studio Codeでのデバッグテスト](#debugging-tests-in-visual-studio-code)
- [アイソレーションにおけるコンポーネントの開発](#developing-components-in-isolation)
  - [Storybook入門](#getting-started-with-storybook)
  - [スタイルガイド入門](#getting-started-with-styleguidist)
- [コンポーネントをnpmに公開する](#publishing-components-to-npm)
- [プログレッシブなWebアプリケーションを作る](#making-a-progressive-web-app)
  - [なぜオプトイン?](#why-opt-in)
  - [オフライン - 最初の考慮事項](#offline-first-considerations)
  - [プログレッシブWebアプリケーションメタデータ](#progressive-web-app-metadata)
- [バンドルサイズの分析](#analyzing-the-bundle-size)
- [デプロイ](#deployment)
  - [静的サーバー](#static-server)
  - [その他のソリューション](#other-solutions)
  - [クライアントサイドルーティングによるアプリ提供](#serving-apps-with-client-side-routing)
  - [相対パスの構築](#building-for-relative-paths)
  - [任意のビルド環境用の環境変数のカスタマイズ](#customizing-environment-variables-for-arbitrary-build-environments)
  - [Azure](#azure)
  - [Firebase](#firebase)
...
- [トラブルシューティング](#troubleshooting)
  ...
  - [`npm test` がmacOS Sierra上でハング、クラッシュ](#npm-test-hangs-or-crashes-on-macos-sierra)
  - [`npm run build` が早く終わる](#npm-run-build-exits-too-early)
  - [`npm run build` がHeroku上で失敗](#npm-run-build-fails-on-heroku)
  - [`npm run build` でminifyが失敗](#npm-run-build-fails-to-minify)
